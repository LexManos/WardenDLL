...
#define CRC32_POLY_ZLIB (0xEDB88320L)
static uint32_t crc32_zlib[256] =
{
...
uint32_t __stdcall crc32_buffer_size(){ return (256 * sizeof(uint32_t)); }
uint32_t __stdcall crc32_seed(){ return 0xFFFFFFFF; }
uint32_t __stdcall crc32_finish(uint32_t crc){ return ~crc; }
uint32_t __stdcall crc32_create(uint32_t polynomial, uint8_t *buffer){
	uint32_t x = 0;
	uint32_t y = 0;
	uint32_t z = 0;
	uint32_t *arr = (uint32_t *)buffer;
	if(polynomial == CRC32_POLY_STD){
		for(x = 0; x < 256; x++)
			arr[x] = crc32_std[x];
		return 0;
	}else if(polynomial = CRC32_POLY_ZLIB){
		for(x = 0; x < 256; x++)
			arr[x] = crc32_zlib[x];
		return 0;
	}else{
...
uint32_t __stdcall crc32_calc(uint8_t *table, uint32_t seed, uint8_t *data, uint32_t len){
	uint32_t x;
	uint32_t crc = seed;
	uint32_t *crc32_table = (uint32_t *)table;
	for(x = 0; x < len; x++)
		crc = (crc << 8) ^ crc32_table[(crc >> 24) ^ data[x]];
	return crc;
}
...
uint32_t __stdcall crc32b_calc(uint8_t *table, uint32_t seed, uint8_t *data, uint32_t len){
	...
	uint32_t *crc32_table = (uint32_t *)table;
	for(x = 0; x < len; x++)
		crc = ((crc >> 8) & 0x00FFFFFF) ^ crc32_table[(crc ^ data[x]) & 0xff];
	return crc;
}